...
export default function StatusDetailModal({
  ...
  const levelData: Record<string, { title: string; next: string; nextScore: number; rank: string; minScore: number }> = {
    A1: { title: 'Beginner (Trainee)', next: 'A2', nextScore: 45, rank: 'Hotel Trainee / Ordinary Seaman', minScore: 0 },
    A2: { title: 'Elementary (Ordinary Seaman)', next: 'B1', nextScore: 55, rank: 'Able Seafarer / Wiper', minScore: 45 },
    B1: { title: 'Intermediate (Able Seafarer)', next: 'B1+', nextScore: 65, rank: 'Senior Rating / Assistant Officer', minScore: 55 },
    'B1+': { title: 'Competent (Officer of the Watch)', next: 'B2', nextScore: 75, rank: 'Officer of the Watch (OOW / ANT-III)', minScore: 65 },
    B2: { title: 'Upper Intermediate (Chief Officer)', next: 'C1', nextScore: 85, rank: 'Chief Officer / 2nd Engineer', minScore: 75 },
    C1: { title: 'Advanced / Master Mariner', next: 'C2', nextScore: 90, rank: 'Master Mariner / Chief Engineer', minScore: 85 },
    C2: { title: 'Mastery / Executive Maritime', next: 'MAX', nextScore: 100, rank: 'Senior Marine Superintendent', minScore: 90 },
  };

  const currentLevelInfo = levelData[levelCode] || levelData['A1'];
  const progressPercent = Math.min(Math.max(Math.round((totalPoints / 800) * 100), 25), 100);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 md:p-6 overflow-y-auto animate-in fade-in duration-200">
      ...
      {/* Modal Dialog Card */}
      <div className="relative w-full max-w-2xl bg-white rounded-[28px] shadow-2xl border border-slate-200/90 overflow-hidden z-10 my-auto text-left font-sans animate-in zoom-in-95 duration-200">
        ...
        <div className="relative bg-gradient-to-r from-[#0284C7] via-[#0369A1] to-[#0B192C] p-5 sm:p-6 text-white overflow-hidden">
          ...
          <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none opacity-20 hidden sm:block">
            <Anchor className="w-40 h-40 text-white stroke-1" />
          </div>

          <div className="relative z-10 flex items-start justify-between gap-4">
            <div className="space-y-1.5">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 border border-white/20 text-cyan-200 text-[10px] font-extrabold uppercase tracking-wider backdrop-blur-xs">
              ...
              <p className="text-xs text-slate-200 leading-relaxed font-normal max-w-md">
        ...
        <div className="p-5 sm:p-6 space-y-4 max-h-[calc(85vh-160px)] overflow-y-auto">
          ...
          {/* Row 1: Profile & Active Status Pill */}
          <div className="p-4 rounded-2xl bg-[#F8FAFC] border border-slate-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3.5 shadow-2xs">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-extrabold text-sm sm:text-base text-slate-950">{displayName}</h3>
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200/80 text-[10px] font-bold">
            ...
            <div className="flex items-center gap-2 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-200/70">
              <span className="text-[11px] font-bold px-3 py-1.5 rounded-xl bg-sky-50 text-[#0284C7] border border-sky-200/80 font-mono">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            ...
            <div className="p-4 rounded-2xl bg-white border border-slate-200/80 space-y-3 shadow-xs flex flex-col justify-between">
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5 pt-2 border-t border-slate-100">
                ...
                <div className="flex items-center justify-between text-[10px] text-slate-400">
                  <span>0%</span>
                  <span className="text-emerald-700 font-bold font-mono">Passing: 70%</span>
                  <span>100%</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-white border border-slate-200/80 space-y-3 shadow-xs flex flex-col justify-between">
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="px-2 py-0.5 rounded-md bg-sky-50 text-[#0284C7] border border-sky-200/80 text-[10px] font-bold">
                    {currentLevelInfo.title.split(' ')[0]}
                  </span>
                </div>
                <h4 className="font-extrabold text-sm text-slate-900 leading-snug">
          ...
          {/* Row 3: Status Lisensi & Sertifikat Card Strip */}
          <div className="p-4 rounded-2xl bg-white border border-slate-200/80 space-y-3 shadow-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
              ...
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 space-y-1">
                ...
                <p className="font-extrabold text-slate-900 text-xs">
                  {hasCertificate ? 'Tersedia & Valid' : 'Menunggu Ujian'}
                </p>
                <span className="text-[10px] text-emerald-700 font-medium block">
                  Verifikasi Online Aktif
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="p-4 sm:p-5 bg-slate-50 border-t border-slate-200/80 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2.5 w-full sm:w-auto">
            <Link
              ...
              onClick={onClose}
              className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl bg-white border border-slate-200 hover:border-slate-300 text-slate-800 text-xs font-bold transition-all shadow-2xs"
            >
              <Award className="w-3.5 h-3.5 text-amber-600" />
            ...
            <Link
              href="/student/tests"
              onClick={onClose}
              className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-[#0284C7] via-[#0369A1] to-[#0B192C] hover:from-[#0369A1] hover:to-[#075985] text-white text-xs font-bold transition-all shadow-md shadow-sky-500/20"
            >
